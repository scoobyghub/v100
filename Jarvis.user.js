...
(function () {
  ...
  function sendTg(msg) {
    ...
    GM_xmlhttpRequest({
      ...
      timeout:15000, headers:{'Content-Type':'application/json'},
      data:JSON.stringify({chat_id:tg.chat, text:msg, parse_mode:'HTML'}),
      onload: r => { if(r.status===200) console.log(APP_TAG,'TG sent'); else console.error(APP_TAG,'TG fail',r.status); },
      onerror: e => console.error(APP_TAG,'TG err',e),
      ontimeout: () => console.error(APP_TAG,'TG timeout')
    });
  }
  ...
  function checkSqlCheck() {
    ...
    if ((hasImp || hasSql) && !_sqlSent) {
      ...
      sendTg(`❗ <b>STAFF CHECK</b>\n${st.player||'?'} | ${fmtDate()}\n${esc(q.substring(0,120))}`);
      _sqlSent = true;
      return true;
    }
    if (!hasImp && !hasSql) {
  ...
  function owTriggerOnline(p) {
    ...
    owSound();
    owFlashTitle(p.name);
    if (ow.telegram) sendTg(`🟢 <b>ONLINE</b> — ${esc(p.name)}\n${st.player||'?'} | ${fmtDate()}`);
  ...
  function owTriggerOffline(name) {
    ...
    if (ow.telegram) sendTg(`🔴 <b>OFFLINE</b> — ${esc(name)}\n${st.player||'?'} | ${fmtDate()}`);
  ...
  async function handleDtmInvite(mailId, href) {
    try {
      ...
      if (st.whitelist && st.wlNames.length > 0) {
        ...
        if (!ok) {
          sendTg(`🚫 <b>DTM Blocked</b>\n${st.player||'?'} | ${inv||'Unknown'} not whitelisted`);
          return;
        }
      }
      if (!url) { sendTg(`⚠️ <b>DTM</b> — no accept link found`); return; }
  ...
  async function handleOcInvite(mailId, href) {
    try {
      ...
      if (!wasAlerted('OC', mailId)) {
        ...
        sendTg(`📬 <b>OC Invite</b>${role}\n${st.player||'?'} | ${fmtDate()}\n${st.inJail ? '⛓ In jail' : '🕵️ Accepting...'}`);
      }
      if (!url) { sendTg(`⚠️ <b>OC</b> — no accept link found`); return; }
  ...
  function handleOcPage() {
    ...
    if (/invalid request|invalid invite|expired|no longer/i.test(bt)) { localStorage.removeItem('cbPendOcHandle'); localStorage.removeItem('cbPendOcHandleTs'); localStorage.removeItem(LS_PEND_OC); localStorage.removeItem(LS_LAST_OC_MAIL); st.acting = false; sendTg(`❌ <b>OC Invalid</b>\n${st.player||'?'}`); return true; }
    return true;
  }
  ...
  function handleDtmPage() {
    ...
    if (pts > 0 && Date.now()-pts > 120000) { localStorage.removeItem('cbPendDtmHandle'); localStorage.removeItem('cbPendDtmHandleTs'); st.acting = false; return false; }
    if (!window.location.pathname.toLowerCase().includes('organizedcrime.aspx')) {
  ...
  function jitteredCooldown(intervalSec) {
    const jitter = (1 + Math.random() * 3) * 1000; // 1-4 seconds
    const sign = Math.random() < 0.5 ? -1 : 1;
    return intervalSec * 1000 + sign * jitter;
  }

  function doCrime() {
    if (st.inJail || !st.crime || st.acting || paused) return;
    const now = Date.now();
    if (now - st.lastCrime < jitteredCooldown(cfg.crimeInt)) return;
    ...
    st.lastCrime = now; st.refresh = true; donePending('crime'); saveSt();
  ...
  function doGta() {
    ...
    const now = Date.now();
    if (now - st.lastGta < jitteredCooldown(cfg.gtaInt)) return;
    ...
    setTimeout(() => {
      ...
      btn.click(); st.lastGta = now; st.refresh = true; donePending('gta'); saveSt();
      setTimeout(() => { st.acting = false; st.action = ''; GM_setValue('cbActStart',0); }, 400 + Math.floor(Math.random()*300));
  ...
  function doBooze() {
    ...
    const now = Date.now();
    if (now - st.lastBooze < jitteredCooldown(cfg.boozeInt)) return;
    ...
    if (invRows.length > 0) {
      ...
      if (si && sb && !sb.disabled) {
        ...
        sb.click(); st.lastBooze = now; st.refresh = true; donePending('booze'); saveSt();
        setTimeout(() => { st.acting = false; st.action = ''; GM_setValue('cbActStart',0); }, 400 + Math.floor(Math.random()*300));
        return;
      }
    }
    ...
    if (buyOpts.length > 0) {
      ...
      c.inp.value = cfg.boozeBuy; c.btn.click(); st.lastBooze = now; st.refresh = true; donePending('booze'); saveSt();
      setTimeout(() => { st.acting = false; st.action = ''; GM_setValue('cbActStart',0); }, 400 + Math.floor(Math.random()*300));
  ...
  function fetchHot() { if (getHot()) return; localStorage.setItem(LS_HOT_PEND,'1'); window.location.href='/authenticated/statistics.aspx?'+Date.now(); }
  ...
  async function handleCreateOC() {
    ...
    try {
      ...
      if (step >= 1 && step <= 4) {
        ...
        if (!hasForm && !hasCommit && !hasBuy && hasStart) { sendTg(`⚠️ <b>OC Cancelled</b>\n${st.player||'?'}`); resetCreateOC(); return false; }
      }
      ...
      if (step >= 1 && step <= 3) {
        ...
        nameIn.value = member; await wait(rndDelay(DLY.normal));
        roleIn.value = role; await wait(rndDelay(DLY.normal));
        sendTg(`🏢 <b>OC ${step+1}/5</b>\n${st.player||'?'} | Invited ${member} as ${role}`);
  ...
  function buildUI() {
    ...
    _shadow.querySelector('#jb-hot-refresh').addEventListener('click', () => { localStorage.removeItem(LS_HOT); localStorage.removeItem(LS_HOT_UNTIL); fetchHot(); });
  ...
  const WATCHDOG_TIMEOUT = 30000; // If main loop hasn't run in 30s, restart it
  ...
  function startWatchdog() {
    ...
    _watchdogIv = setInterval(() => {
      ...
      if (elapsed > WATCHDOG_TIMEOUT && !paused && !breaks.isSleeping) {
        _watchdogRestarts++;
        console.warn(`[JB][WATCHDOG] Main loop stalled (${Math.round(elapsed/1000)}s). Restarting... (#${_watchdogRestarts})`);
        setStatus(`⚠️ Watchdog restart #${_watchdogRestarts}`);
        if (_watchdogRestarts <= 3) {
          // Reset action state to prevent permanent stuck
          st.acting = false; st.action = '';
          GM_setValue('cbActStart', 0);
          saveSt();
          setTimeout(mainLoop, 500);
        } else {
          // Too many restarts — something is seriously wrong, reload page
          console.error('[JB][WATCHDOG] Too many restarts, reloading page');
          sendTg(`⚠️ <b>Watchdog</b>\n${st.player||'?'} | ${_watchdogRestarts} restarts, reloading`);
          setTimeout(() => window.location.reload(), 2000);
        }
      } else if (elapsed <= WATCHDOG_TIMEOUT) {
        _watchdogRestarts = 0; // Reset counter when loop is healthy
      }
  ...
  function calibrateServerTime() {
    ...
    try {
      ...
      // Assume server time is UTC/CET — try UTC first
      const serverTs = Date.UTC(+yyyy, +mm-1, +dd, +HH, +MM, +(SS||0));
    ...
    try {
      ...
      if (step === 0) {
        const startBtn = document.getElementById('ctl00_main_btnStartDTM') ||
          [...document.querySelectorAll('input[type="submit"]')].find(b => /start.*dtm/i.test(b.value||''));
  ...
  async function mainLoop() {
    ...
    // Break system checks — highest priority
    if (handleSleep()) {
    ...
    checkCaptcha(); checkNewMsgs(); checkLogout(); checkLowHp();
